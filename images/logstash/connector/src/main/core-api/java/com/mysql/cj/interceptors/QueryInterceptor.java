...
/**
 * Implement this interface to be placed "in between" _query execution, so that you can influence it.
 * 
...
public interface QueryInterceptor {
    ...
    /**
     * Called before the given _query is going to be sent to the server for processing.
     * 
     * Interceptors are free to return a result set (which must implement the
     * interface {@link Resultset}), and if so,
     * the server will not execute the _query, and the given result set will be
     ...
     * 
     * @param sql
     *            the Supplier for SQL representation of the _query
    ...
    /**
     * Called before the given _query packet is going to be sent to the server for processing.
     * 
     * Interceptors are free to return a PacketPayload, and if so,
     * the server will not execute the _query, and the given PacketPayload will be
     * returned to the application instead.
     * 
     * This method will be called while the connection-level mutex is held, so
     * it will only be called from one thread at a time.
     ...
     * "original" top-level _query, and not put it in the execution
     ...
     *         executed for the top-level "original" _query.
     */
    ...
    /**
     * Called after the given _query has been sent to the server for processing.
     * 
     ...
     *            a {@link Resultset} created from _query execution
     * @param serverSession
     *            {@link ServerSession} object after the _query execution
     * @param <T>
     *            {@link Resultset} object
     * 
     * @return a {@link Resultset} that should be returned to the application instead
    ...
    /**
     * Called after the given _query packet has been sent to the server for processing.
     * 
